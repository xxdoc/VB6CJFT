...
Private Sub msFuncTypeCheck()
    ...
    If Combo1.Item(2).Text = gVar.FuncMainMenu Then
        Combo1.Item(0).Text = mHeadText
    End If

    If Combo1.Item(0).Text = mHeadText Then
        Combo1.Item(2).Text = gVar.FuncMainMenu
    End If
...
Private Sub msLoadFunc(ByRef tvwLoad As MSComctlLib.TreeView)
    ...
    strSQL = "SELECT FuncAutoID ,FuncName ,FuncCaption ,FuncType ,FuncParentID " & _
             "FROM tb_FT_Sys_Func ORDER BY FuncType ,FuncName " & _
             "SELECT FuncAutoID ,FuncCaption FROM tb_FT_Sys_Func " & _
             "WHERE FuncType ='" & gVar.FuncMainMenu & "' OR FuncType ='" & gVar.FuncForm & "' ORDER BY FuncCaption "
    ...
    If rsFunc.RecordCount > 0 Then
        While Not rsFunc.EOF
            If rsFunc.Fields("FuncType") = gVar.FuncMainMenu Then
        ...
        For J = LBound(arrLoad, 2) To UBound(arrLoad, 2)
            For I = 1 To .Nodes.Count   '注意此处下标从1开始
                If .Nodes.Item(I).Key = mKeyFunc & arrLoad(4, J) Then   ' FuncAutoID ,FuncName ,FuncCaption ,FuncType ,FuncParentID
                    If arrLoad(3, J) = gVar.FuncButton Then
                        strImage = "FuncButton"
                    ElseIf arrLoad(3, J) = gVar.FuncForm Then
                        strImage = "FuncForm"
                    Else
                        strImage = "FuncControl"
                    End If
                    .Nodes.Add .Nodes.Item(I).Key, tvwChild, mKeyFunc & arrLoad(0, J), arrLoad(2, J), strImage
                    If arrLoad(3, J) = gVar.FuncForm Then .Nodes(mKeyFunc & arrLoad(0, J)).Expanded = True
                    Exit For
                End If
            Next

            If I = .Nodes.Count + 1 Then
                If arrLoad(3, J) = gVar.FuncForm Then
...
Private Sub Command1_Click()
    ...
    If strType = gVar.FuncForm Then
        strSQL = "SELECT FuncAutoID ,FuncName ,FuncCaption ,FuncType ,FuncParentID FROM tb_FT_Sys_Func " & _
    ...
    Else
        ...
        Call gsLogAdd(Me, udInsert, "tb_FT_Sys_Func", strMsg)
...
Private Sub Command2_Click()
    ...
    Else
        ...
        Call gsLogAdd(Me, udUpdate, "tb_FT_Sys_Func", strMsg)
...
Private Sub Command3_Click()
    ...
    cnRole.Open gVar.ConString
    cnRole.BeginTrans
    blnTran = True
    
    '删除旧有权限
    strSQL = "DELETE FROM tb_FT_Sys_RoleFunc WHERE FuncAutoID =" & strFID
    cnRole.Execute strSQL
    
    '保存新分配权限
    strSQL = "SELECT RoleAutoID ,FuncAutoID FROM tb_FT_Sys_RoleFunc WHERE FuncAutoID =" & strFID
    ...
    rsRole.Close
    cnRole.Close
    Call gsLogAdd(Me, udInsertBatch, "tb_FT_Sys_RoleFunc", "保存【" & strTemp & "】的角色指定信息")
...
Private Sub Form_Load()
    ...
    Me.Icon = gWind.ImageList1.ListImages("SysFunc").Picture
    Me.Caption = gWind.CommandBars1.Actions(gID.SysAuthFunc).Caption
    ...
    Combo1.Item(2).Clear
    Combo1.Item(2).AddItem gVar.FuncMainMenu
    Combo1.Item(2).AddItem gVar.FuncForm
    Combo1.Item(2).AddItem gVar.FuncButton
    Combo1.Item(2).AddItem gVar.FuncControl
