...
Private Sub Command1_Click()
    ...
    strSQL = "SELECT UserLoginName ,UserPassword ,UserFullName FROM tb_FT_Sys_User " & _
    ...
    If rsEdit.Fields("UserFullName") <> strName Then
        ...
        GoTo LineEnd
    End If
    If DecryptString(rsEdit.Fields("UserPassword"), gVar.EncryptKey) <> strPwdPre Then '解密数据库中的密码
    ...
    If MsgBox("确定修改当前密码吗？", vbQuestion + vbOKCancel, "密码修改询问") = vbOK Then
        rsEdit.Fields("UserPassword") = EncryptString(strPwdNew, gVar.EncryptKey)
        rsEdit.Update
        rsEdit.Close
        Call gsLogAdd(Me, udUpdate, "tb_FT_Sys_User", "修改用户ID【" & strULID & "】的登陆密码")
...
Private Sub Form_Load()
    '
    Set Me.Icon = gWind.ImageList1.ListImages("SysPassword").Picture
    Text1.Item(0).Text = gVar.UserFullName
    Text1.Item(1).Text = gVar.UserLoginName
