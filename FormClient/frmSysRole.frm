...
Private Sub msLoadFunc(ByRef tvwLoad As MSComctlLib.TreeView)
    ...
    strSQL = "SELECT FuncAutoID ,FuncName ,FuncCaption ,FuncType ,FuncParentID " & _
             "FROM tb_FT_Sys_Func ORDER BY FuncType ,FuncName "
...
Private Sub Combo1_Click(Index As Integer)
    ...
    strSQL = "SELECT RoleAutoID ,FuncAutoID FROM tb_FT_Sys_RoleFunc " & _
    ...
    Call gsLogAdd(Me, udInsertBatch, "tb_FT_Sys_RoleFunc", "将角色【" & strLeadID & mTwoBar & strLeadText & "】的权限导入给角色【" & strTemp & "】")
...
Private Sub Command1_Click()
    ...
    strSQL = "SELECT RoleAutoID ,RoleName ,DeptID FROM tb_FT_Sys_Role WHERE 1<>1 "
    ...
    If rsRole.State = adStateOpen Then
        ...
        Call gsLogAdd(Me, udInsert, "tb_FT_Sys_Role", "添加新角色【" & lngRID & "】【" & strRoleName & "】")
...
Private Sub Command2_Click()
    ...
    If rsRole.State = adStateOpen Then
        If rsRole.RecordCount = 1 Then
            ...
            Call msLoadDept(TreeView1)
            Call msLoadRole(TreeView1)
            Call gsLogAdd(Me, udUpdate, "tb_FT_Sys_Role", strMsg)
...
Private Sub Command3_Click()
    ...
    '删除旧有权限
    strSQL = "DELETE FROM tb_FT_Sys_RoleFunc WHERE RoleAutoID =" & strRID
    cnRole.Execute strSQL
    
    '保存新分配权限
    strSQL = "SELECT RoleAutoID ,FuncAutoID FROM tb_FT_Sys_RoleFunc WHERE RoleAutoID =" & strRID
    rsRole.Open strSQL, cnRole, adOpenStatic, adLockBatchOptimistic
    If rsRole.RecordCount > 0 Then
    ...
    rsRole.Close
    cnRole.Close
    Call gsLogAdd(Me, udInsertBatch, "tb_FT_Sys_RoleFunc", "保存【" & strTemp & "】的权限分配信息")
...
Private Sub Form_Load()
    ...
    Me.Icon = gWind.ImageList1.ListImages("SysRole").Picture
    Me.Caption = gWind.CommandBars1.Actions(gID.SysAuthRole).Caption
...
Private Sub TreeView1_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    strSQL = "SELECT RoleAutoID ,RoleName ,DeptID FROM tb_FT_Sys_Role " & _
             "WHERE RoleAutoID =" & strRID
    Set rsRole = gfBackRecordset(strSQL)
    If rsRole.State = adStateClosed Then GoTo LineEnd
    If rsRole.RecordCount = 0 Then
